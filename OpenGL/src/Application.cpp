...
#define GLCall(x) GLClearError();\
    x;\
    ASSERT(GLLogCall(#x, __FILE__, __LINEjj__))
...
int main(void)
{
    ...
    window = glfwCreateWindow(640, 480, "ZacStack.dev", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    unsigned int buffer;
    GLCall(glGenBuffers(numOfBuffers, &buffer));
    ...
    GLCall(glGenBuffers(numOfBuffers, &ibo));
